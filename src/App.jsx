...
import { Moon, Clock, Sun, FileText, X, LogOut } from 'lucide-react'
  ...
  return (
    <div className="px-4 py-4 bg-slate-900">
      <div className="flex justify-between items-center mb-3">
        <span className="text-xs font-medium text-slate-400 flex items-center gap-1.5"><Moon className="w-3.5 h-3.5" /> Day Progress</span>
...
export default function App() {
  ...
  async function saveNote() {
    ...
    const payload = editing 
      ? { ...form, done: editing?.done || false, updated_at: new Date().toISOString() }
      : { ...form, done: false }
    await fetch(url, {
      method,
  ...
  const plain = notes.filter(n => !n.reminder_at)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white/90 backdrop-blur-xl rounded-3xl p-10 shadow-2xl shadow-slate-900/10 border border-slate-200/50">
          <h1 className="text-3xl font-bold text-slate-900 mb-2 text-center">{screen === 'login' ? 'Welcome back' : 'Create account'}</h1>
          <p className="text-sm text-slate-500 text-center mb-8">{screen === 'login' ? 'Sign in to your space' : 'Get started with your space'}</p>
          ...
          <p className="text-center text-sm text-slate-500 mt-8">
            {screen === 'login'
              ? <span onClick={() => { setScreen('register'); setError('') }} className="cursor-pointer text-slate-900 hover:text-slate-700 font-semibold">No account? Register</span>
              : <span onClick={() => { setScreen('login'); setError('') }} className="cursor-pointer text-slate-900 hover:text-slate-700 font-semibold">Have an account? Login</span>
            }
          </p>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 flex flex-col">
      ...
      <div className="bg-slate-900/95 backdrop-blur-lg border-b border-slate-700/50 px-6 pt-12 pb-5 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1 font-medium">{new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
        </div>
        <button onClick={() => setShowLogoutConfirm(true)} className="text-sm text-slate-300 hover:text-white transition font-medium flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-slate-800">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3 pb-24">
        {plain.length === 0
          ...
          : plain.map(note => (
            <div key={note.id} className="bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200/60 p-5 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50 transition-all duration-300">
              ...
              {note.body && <p className="text-sm text-slate-600 mt-3 leading-relaxed">{note.body}</p>}
              <div className="text-xs text-slate-400 mt-4 font-medium">
                <span className="block">Created: {new Date(note.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                {note.updated_at && <span className="block">Last modified: {new Date(note.updated_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>}
              </div>
            </div>
          ))
        }
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-40 flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl w-full max-w-lg p-8 space-y-6 shadow-2xl shadow-slate-900/20 border border-slate-200/50">
            <h3 className="font-bold text-slate-900 text-xl">{editing ? 'Edit' : 'New note'}</h3>
            ...
            <div className="flex gap-4 pt-2">
              <button onClick={() => { setShowForm(false); setEditing(null); setForm({ title: '', body: '' }) }}
      ...
      <button onClick={() => {
        ...
        className="fixed bottom-8 right-8 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-full w-18 h-18 text-4xl shadow-2xl shadow-slate-900/30 hover:from-slate-800 hover:to-slate-700 transition-all hover:scale-105 z-30 flex items-center justify-center w-16 h-16">
        +
      </button>
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl w-full max-w-sm p-8 shadow-2xl shadow-slate-900/20 border border-slate-200/50">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => { setToken(null); setScreen('login'); setShowLogoutConfirm(false) }}
                className="flex-1 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl py-4 text-sm font-semibold hover:from-slate-800 hover:to-slate-700 transition-all shadow-lg shadow-slate-900/20">Logout</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl w-full max-w-sm p-8 shadow-2xl shadow-slate-900/20 border border-slate-200/50">
            ...
            <p className="text-sm text-slate-600 mb-7">
              {noteToDelete.done 
                ? `Do you want to remove completed event "${noteToDelete.title}"?`
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => { deleteNote(noteToDelete.id); setShowDeleteConfirm(false); setNoteToDelete(null) }}
                className="flex-1 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-2xl py-4 text-sm font-semibold hover:from-red-600 hover:to-red-700 transition-all shadow-lg shadow-red-500/20">Delete</button>
            </div>
          </div>
        </div>
      )}
